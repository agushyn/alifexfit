...
export default function WorkoutEdit({ workoutType }: WorkoutEditProps) {
    const { data, setData, put, processing, errors } = useForm({
        name: workoutType.name,
        category: workoutType.category || 'Strength',
        description: workoutType.description || '',
        status: workoutType.status,
        sort_order: workoutType.sort_order,
    });

    const submit: FormEventHandler = (e) => {
        e.preventDefault();
        put(route('admin.workout-types.update', workoutType.id));
    };
    ...
    return (
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        <h1 className="text-2xl font-extrabold text-white tracking-tight flex items-center gap-2.5">
                            ...
                            Edit Workout Type: {workoutType.name}
                        </h1>
                        <p className="text-xs text-[#888888] mt-1">
                            Modify exercise discipline category and details for this branch.
                        </p>
                    </div>
        ...
        >
            <Head title={`Edit ${workoutType.name} — EXFITS Gym`} />
